<template>
  <nav ref="navbarRef" class="navbar navbar-expand-lg custom-navbar">
...
const handleScroll = () => {
  ...
  navbarRef.value.style.transition = 'transform 0.5s ease, box-shadow 0.5s ease'

  // Case 1: At the very top
  if (currentScroll === 0) {
    navbarRef.value.style.transform = 'translateY(0)'
    navbarRef.value.style.boxShadow = 'none'
    lastScroll.value = currentScroll
    return
  }

  // Case 2: Scrolling DOWN → hide navbar
  if (currentScroll > lastScroll.value) {
    navbarRef.value.style.transform = 'translateY(-100%)'
    navbarRef.value.style.boxShadow = 'none'
  }
  // Case 3: Scrolling UP → show navbar with shadow
  else if (currentScroll < lastScroll.value) {
    navbarRef.value.style.transform = 'translateY(0)'
    navbarRef.value.style.boxShadow = '0 4px 12px rgba(212, 76, 255, 0.3)'
  }
